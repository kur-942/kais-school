...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  const { user } = useAuth();
  const { categories, isLoading, fetchCategoriesByNiveau, selectCategory } =
    useCategoryStore();
        ...
        <div className="p-5">
          ...
          <div className="mb-6">
            ...
            >
              ...
              <button
                type="submit"
                className="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-sm hover:shadow-md sm:w-auto w-full"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
          ...
          <div className="mb-6">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                >
                  <svg
                    className={`w-4 h-4 ${!filters.categoryId ? "text-white" : "text-gray-600"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <span className="text-xs text-gray-500">
                {categories.reduce(
                  (acc, cat) => acc + (cat.course_count || 0),
                  0,
                )}
              </span>
            </button>
